...
document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        // รอให้ jQuery และ DataTables โหลดเสร็จก่อนเริ่มต้น
...
// ฟังก์ชันรอให้ libraries โหลดเสร็จ
...
async function loadCoursesForEditForm() {
    try {
        ...
        const select = document.getElementById('editContentCourse');
        if (select) {
            // เก็บค่า selected ปัจจุบัน
            ...
            querySnapshot.forEach((doc) => {
                ...
                // ตั้งค่า selected ถ้าเป็นคอร์สที่เลือกไว้
...
function collectQuizData() {
    ...
    // รวบรวมแผนกที่สามารถทำข้อสอบได้
...
async function loadContent() {
    try {
        ...
        console.log('ข้อมูลเนื้อหาที่โหลดมา:', contentArray);
        
        contentArray.forEach((content) => {
            console.log('กำลังสร้างแถวสำหรับเนื้อหา:', content.title);
            ...
            // สร้างเซลล์แต่ละคอลัมน์แยกกันเพื่อให้แน่ใจว่าข้อมูลอยู่ในคอลัมน์ที่ถูกต้อง
            ...
            typeCell.innerHTML = `<span class="badge bg-secondary">${typeText}</span>`;
            ...
            descCell.className = 'table-cell-truncate';
            descCell.title = content.description || '-';
            descCell.textContent = truncateText(content.description || '-', 50);
            
            const courseCell = document.createElement('td');
            courseCell.textContent = courseName;
            
            const dateCell = document.createElement('td');
            dateCell.textContent = formatDate(content.createdAt);
        ...
        // รอให้ libraries โหลดเสร็จก่อนเริ่มต้น DataTable
        await waitForLibraries();
        ...
        const contentDataTableConfig = {
            ...
            pageLength: 10,
            order: [[5, 'desc']], // เรียงตามวันที่สร้าง (คอลัมน์ที่ 5)
            autoWidth: false,
            scrollX: false,
            responsive: true,
            columnDefs: [
        ...
        if (typeof $.fn.dataTable.Responsive !== 'undefined') {
            contentDataTableConfig.responsive = true;
        }
        
        // ตรวจสอบว่า jQuery และ DataTables โหลดแล้ว
        if (typeof $ !== 'undefined' && $.fn.DataTable) {
            try {
                window.contentDataTable = $('#contentTable').DataTable(contentDataTableConfig);
            } catch (error) {
                console.error('เกิดข้อผิดพลาดในการเริ่มต้น DataTable:', error);
            }
        } else {
            console.warn('jQuery หรือ DataTables ยังไม่โหลดเสร็จ');
        }
    } catch (error) {
...
async function editContent(contentId) {
    try {
        ...
        // โหลดข้อมูลคอร์สและตั้งค่า selected
        await loadCoursesForEditForm();
...
function setupDepartmentSelection() {
    ...
    // ปุ่มล้างทั้งหมด
    const clearAllBtn = document.getElementById('clearAllDepartments');
    if (clearAllBtn) {
