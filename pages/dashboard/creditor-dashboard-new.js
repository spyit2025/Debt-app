...
function loadDashboardDataInternal() {
    ...
    debtsRef.get().then(function(querySnapshot) {
        ...
        if (debtsData.length === 0) {
            console.log('No debts found for this creditor');
            ...
            .then(function(doc) {
                if (doc.exists) {
                    ...
                    const updatedPaymentHistory = paymentHistory.filter(payment => {
                        const currentPaymentId = payment.paymentId || payment.timestamp;
    ...
    window.firebaseDb.collection('debts').where('creditorId', '==', userId).get()
        .then(function(querySnapshot) {
            ...
            if (paymentHistoryData.length === 0) {
                console.log('No payment history found for user:', userId);
                const tableBody = document.getElementById('paymentHistoryTableBody');
                if (tableBody) {
                    tableBody.innerHTML = `
                        <tr>
...
function populateDebtorFilter() {
    ...
    console.log(`Loaded ${debtors.length} debtor options for payment history filter (updated ${debtorFilters.length} elements)`);
}
...
function createPaymentHistoryMobileCards() {
    ...
    if (filteredPaymentHistory.length === 0) {
        ...
        `;
        console.log('No payment history data, showing empty message');
        return;
    }
...
function closeAddDebtModal() {
    console.log('Closing add debt modal...');
    
    const addDebtModal = document.getElementById('addDebtModal');
    if (!addDebtModal) {
        console.log('Add debt modal not found');
        return;
    }
    ...
    try {
        ...
        if (bootstrapModal) {
            ...
            console.log('No Bootstrap modal instance, using manual cleanup');
        ...
        console.log('Add debt modal closed successfully');
...
function loadDebtorFilterOptions() {
    ...
    console.log(`Loaded ${debtors.length} debtor options for filter`);
}
